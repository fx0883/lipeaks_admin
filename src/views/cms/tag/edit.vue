...
const fetchTagDetail = async () => {
  try {
    loading.value = true;
    await cmsStore.fetchTagDetail(tagId.value);
    const tag = cmsStore.currentTag;
...
const submitForm = async () => {
  ...
  await formRef.value.validate(async valid => {
    if (valid) {
      try {
        loading.value = true;
        await cmsStore.updateTag(tagId.value, form);
        ElMessage.success(t("cms.tag.updateSuccess"));
        goBack();
      } catch (error) {
        console.error("Failed to update tag:", error);
        ElMessage.error(t("cms.tag.updateFailed"));
      } finally {
        loading.value = false;
      }
    }
