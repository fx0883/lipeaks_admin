<template>
  <div class="main-container">
    ...
    <div class="form-header">
      ...
      <h2>{{ $t("license.licenses.editTitle") || "许可证信息更新" }}</h2>
      <p class="form-description">{{ $t("license.licenses.editDescription") || "请填写需要更新的许可证信息" }}</p>
    ...
    <!-- 许可证更新表单 -->
    <form id="licenseUpdateForm" class="license-update-form" v-loading="loading">
      <el-form
        ref="formRef"
        :model="form"
        :rules="rules"
      ...
      >
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              >
                ...
                <div v-if="tenantLocked" class="field-help">
                  {{ $t("license.licenses.tenantLocked") || "租户已锁定，无法修改" }}
                </div>
                <div class="field-error" id="tenant_error"></div>
              </el-form-item>
            </div>
          </div>
        ...
        <div class="form-section">
          ...
          <div class="form-row">
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <span style="float: right; color: #8492a6; font-size: 13px">v{{ product.version }}</span>
                  </el-option>
                </el-select>
                <div class="field-help" v-if="productLocked">
                  {{ $t("license.licenses.productLockedByPlan") || "产品已被方案锁定" }}
                </div>
            ...
            <div class="form-group">
              <el-form-item :label="$t('license.licenses.plan')" prop="plan">
                <el-select
                  v-model="form.plan"
        ...
        <div class="form-actions">
          ...
          <el-button
            ...
            class="btn-primary"
          >
            <span v-if="!submitting" class="btn-text">
              <el-icon><Check /></el-icon>
              {{ $t("license.licenses.updateLicense") || "更新许可证" }}
            </span>
            <span v-else class="btn-loading">
              <el-icon class="is-loading"><Loading /></el-icon>
              {{ $t("license.licenses.updating") || "更新中..." }}
            </span>
          </el-button>
        </div>
...
const loadAllPlans = async () => {
  try {
    ...
    const response = await getPlanList({ page_size: 1000 });
...
const loadAvailableTenants = async () => {
  try {
    ...
    const response = await getTenantList({ page_size: 1000 });
...
const loadLicenseData = async () => {
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      // 设置级联选择状态
      if (form.plan) {
        const selectedPlan = allPlans.value.find(p => p.id === form.plan);
        if (selectedPlan) {
          availablePlans.value = allPlans.value.filter(
            plan => plan.product === selectedPlan.product
          );
          if (form.product !== selectedPlan.product) {
            form.product = selectedPlan.product;
...
const handlePlanChange = (planId: number | null) => {
  if (planId && !productLocked.value) {
    ...
    if (selectedPlan && selectedPlan.product !== form.product) {
      form.product = selectedPlan.product;
      productLocked.value = true;
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (form.plan) submitData.plan = form.plan;
    if (form.tenant) submitData.tenant = form.tenant;
