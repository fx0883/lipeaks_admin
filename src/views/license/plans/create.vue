    ...
    <el-card v-loading="loading" :header="false" shadow="never">
      <template #header>
        <el-breadcrumb separator="/">
          ...
          <el-breadcrumb-item>{{ $t('license.plans.create') }}</el-breadcrumb-item>
        </el-breadcrumb>
      </template>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    ElMessage.success(t('license.plans.createSuccess'))
    router.push('/license/plans')
  } catch (error) {
    console.error('Create plan failed:', error)
    ElMessage.error(t('license.plans.createFailed'))
  } finally {
