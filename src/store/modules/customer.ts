...
interface CustomerState {
  ...
  // 按客户ID存储的联系人关系列表映射
  customerMemberRelationsMap: Record<
    number,
    PaginationData<CustomerMemberRelation>
  >;
  // 当前客户的租户关系列表
...
export const useCustomerStore = defineStore("customer", {
  ...
  getters: {
    ...
    // 获取加载状态
    isLoading: state => (key: keyof CustomerState["loading"]) =>
      state.loading[key],
  ...
  actions: {
      ...
      try {
        ...
        if (response.success) {
          ...
          if (response.data && "results" in response.data) {
            ...
            this.customerList.data = Array.isArray(response.data)
              ? response.data
              : [];
          }
          return response;
     ...
     * 创建客户
     */
    async createCustomer(data: CustomerCreateUpdateParams) {
    ...
    async updateCustomerInfo(id: number, data: CustomerCreateUpdateParams) {
      ...
      try {
        ...
        if (response.success) {
          ...
          // 更新客户列表中的客户信息
          const index = this.customerList.data.findIndex(
    ...
    async removeCustomer(id: number) {
      ...
      try {
        ...
        if (response.success) {
          ...
          // 从客户列表中移除被删除的客户
          this.customerList.data = this.customerList.data.filter(
            customer => customer.id !== id
          );
    ...
    async bulkCreateCustomers(data: CustomerCreateUpdateParams[]) {
      ...
      try {
        ...
        if (response.success) {
          ElMessage.success(
            response.message || `成功创建 ${response.data.success_count} 个客户`
          );
          return response;
        } else {
    ...
    async bulkUpdateCustomers(data: CustomerBulkOperationParams) {
      ...
      try {
        ...
        if (response.success) {
          ElMessage.success(
            response.message || `成功更新 ${response.data.success_count} 个客户`
          );
    ...
    async bulkDeleteCustomers(customerIds: number[]) {
      ...
      try {
        ...
        if (response.success) {
          ElMessage.success(
            response.message || `成功删除 ${response.data.success_count} 个客户`
          );

          // 如果当前选中的客户在被删除的客户列表中，则清空当前选中的客户
          if (
            this.currentCustomer &&
            customerIds.includes(this.currentCustomer.id)
          ) {
            this.currentCustomer = null;
          }
     ...
     * 获取客户的联系人关系列表
     */
    async fetchCustomerMemberRelations(
      customerId: number,
      ...
      try {
        ...
        if (response.success) {
          ...
          const memberRelations = response.data.map(member => ({
            ...
            remarks:
              member.relation?.remarks !== null
                ? member.relation?.remarks || ""
                : "",
     ...
     * 创建客户-联系人关系
     */
    async createCustomerMemberRelation(
      ...
      try {
        ...
        if (response.success) {
          ...
          this.customerMemberRelationsMap[customerId].data.unshift(
            response.data
          );
     ...
     * 更新客户-联系人关系
     */
    async updateCustomerMemberRelation(
      customerId: number,
      relationId: number,
      ...
      try {
        const response = await updateCustomerMemberRelation(
          customerId,
          relationId,
          data
        );
        if (response.success) {
          // 更新关系列表中的关系信息
          const index = this.customerMemberRelations.data.findIndex(
          ...
          // 更新客户关系映射中的关系信息
          if (this.customerMemberRelationsMap[customerId]) {
            const mapIndex = this.customerMemberRelationsMap[
              customerId
            ].data.findIndex(relation => relation.id === relationId);
            if (mapIndex !== -1) {
              this.customerMemberRelationsMap[customerId].data[mapIndex] =
                response.data;
            }
          }
    ...
    async removeCustomerMemberRelation(customerId: number, relationId: number) {
      ...
      try {
        const response = await deleteCustomerMemberRelation(
          customerId,
          relationId
        );
        if (response.success) {
          // 从关系列表中移除被删除的关系
          this.customerMemberRelations.data =
            this.customerMemberRelations.data.filter(
              relation => relation.id !== relationId
            );
          this.customerMemberRelations.total--;
          ...
          // 从客户关系映射中移除被删除的关系
          if (this.customerMemberRelationsMap[customerId]) {
            this.customerMemberRelationsMap[customerId].data =
              this.customerMemberRelationsMap[customerId].data.filter(
                relation => relation.id !== relationId
              );
            this.customerMemberRelationsMap[customerId].total--;
          }
    ...
    async setPrimaryMemberRelation(customerId: number, relationId: number) {
      ...
      try {
        ...
        if (response.success) {
          // 更新关系列表中的关系信息
          this.customerMemberRelations.data =
            this.customerMemberRelations.data.map(relation => ({
              ...relation,
          ...
          if (this.customerMemberRelationsMap[customerId]) {
            this.customerMemberRelationsMap[customerId].data =
              this.customerMemberRelationsMap[customerId].data.map(
                relation => ({
                  ...relation,
                  is_primary: relation.id === relationId
                })
     ...
     * 获取客户的租户关系列表
     */
    async fetchCustomerTenantRelations(
      customerId: number,
      params: { page?: number; page_size?: number } = {}
    ) {
      ...
      try {
        ...
        if (response.success) {
          ...
          if (response.data && "results" in response.data) {
            ...
            this.customerTenantRelations.data = Array.isArray(response.data)
              ? response.data
              : [];
          }
          return response;
     ...
     * 创建客户-租户关系
     */
    async createCustomerTenantRelation(
     ...
     * 更新客户-租户关系
     */
    async updateCustomerTenantRelation(
      customerId: number,
      relationId: number,
      ...
      try {
        const response = await updateCustomerTenantRelation(
          customerId,
          relationId,
          data
        );
        if (response.success) {
          // 更新关系列表中的关系信息
          const index = this.customerTenantRelations.data.findIndex(
            relation => relation.id === relationId
          );
          if (index !== -1) {
    ...
    async removeCustomerTenantRelation(customerId: number, relationId: number) {
      ...
      try {
        const response = await deleteCustomerTenantRelation(
          customerId,
          relationId
        );
        if (response.success) {
          // 从关系列表中移除被删除的关系
          this.customerTenantRelations.data =
            this.customerTenantRelations.data.filter(
              relation => relation.id !== relationId
            );
          this.customerTenantRelations.total--;
    ...
    async setPrimaryTenantRelation(customerId: number, relationId: number) {
      ...
      try {
        ...
        if (response.success) {
          // 更新关系列表中的关系信息
          this.customerTenantRelations.data =
            this.customerTenantRelations.data.map(relation => ({
              ...relation,
              is_primary: relation.id === relationId
            }));
    ...
    async batchDeleteCustomerMemberRelations(relationIds: number[]) {
      ...
      try {
        ...
        if (response.success) {
          ...
          this.customerMemberRelations.total -=
            response.data.success_count || relationIds.length;
          ...
          // 按客户ID分组
          const customerIds = [
            ...new Set(relationsToDelete.map(relation => relation.customer.id))
          ];
     ...
     */
    async batchDeleteCustomerMembersByIds(
      customerId: number,
      memberIds: number[]
    ) {
      this.loading.batchDeleteMemberRelations = true;
      ...
      try {
        const response = await batchDeleteCustomerMembersByIds(
          customerId,
          memberIds
        );
        if (response.success) {
          ...
          const relationsToDelete = this.customerMemberRelations.data.filter(
            relation =>
              relation.customer.id === customerId &&
              memberIds.includes(relation.member.id)
          );
