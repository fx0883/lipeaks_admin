...
const emit = defineEmits(["update:coverImage", "update:coverImageSmall"]);
...
const handleUpload: UploadProps["httpRequest"] = async ({ file }) => {
  if (file instanceof File) {
    loading.value = true;
    try {
      const response = await cmsStore.uploadCoverImageWithThumbnail(file, props.folder);
      if (response && response.url && response.thumbnail_url) {
        emit("update:coverImage", response.url);
        emit("update:coverImageSmall", response.thumbnail_url);
