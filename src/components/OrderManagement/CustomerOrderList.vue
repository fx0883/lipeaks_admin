...
const statistics = computed(() => {
  if (!customerOrders.value.length) return null;

  const totalAmount = customerOrders.value.reduce((sum, order) => {
    const amount = parseFloat(order.customer_total_amount || "0");
    return sum + amount;
  }, 0);

  const statusCount = customerOrders.value.reduce(
    (acc, order) => {
  ...
  return {
    totalCount: customerOrders.value.length,
...
const loadCustomerOrders = async () => {
  ...
  try {
    ...
    await orderStore.fetchCustomerOrderList(props.customerId, params);
    customerOrders.value = orderStore.getCustomerOrders(props.customerId);
    pagination.value.total =
      orderStore.customerOrdersMap[props.customerId]?.total || 0;
  } catch (error) {
    console.error("Failed to load customer orders:", error);
    ElMessage.error(t("order.customerOrdersLoadFailed"));
  } finally {
    loading.value = false;
  }
};
...
const createOrder = () => {
  router.push({
    path: "/order/create",
    query: { customer_id: props.customerId.toString() }
  });
};
